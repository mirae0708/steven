---
title: "Context — 공식 팀 지식"
created: 2026-05-23
status: approved
tags: [context/]
---

# 🏛️ Context — 공식 팀 지식

> 이 폴더는 팀 전체가 합의한 **공식 지식**이 있는 곳입니다.
> 모든 문서는 PR 승인을 거쳐 등록됩니다.

## 📁 Company — 회사/팀
- [[직원용-코딩-아키텍처-가이드|📐 직원용 코딩 & 아키텍처 가이드]]

## 📦 Product — 제품/시스템
- [[../Product/시스템-아키텍처|🏗️ 시스템 아키텍처]]
- [[../Product/AI-Council-분석방법론|🧠 AI Council 분석 방법론]]
- [[../Product/자율형-에이전트-로드맵|🚀 자율형 에이전트 로드맵]]
- [[../Product/Quote-Tool-연동가이드|📦 Freight Quote Tool 연동가이드]]
- [[../Product/텔레그램-명령어-모음|📱 텔레그램 명령어 모음]]

## 🔧 TechStack — 기술 스택
- [[../TechStack/물류-ERP-연동가이드|📋 물류 ERP 연동가이드 (MRCloud + Invoice Auto)]]
- [[../TechStack/WSL-문제해결-가이드|🛠️ WSL 환경 문제 해결 가이드]]

---

> 💡 **새 지식 제안?** Members/에 초안 작성 → `@Hermes: 이 문서를 Context로 승격시켜줘`
